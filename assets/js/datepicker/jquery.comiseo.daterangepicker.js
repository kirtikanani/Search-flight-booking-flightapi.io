...
(function($, window, undefined) {
	...
	$.widget('comiseo.daterangepicker', {
		...
		options: {
			...
			cancelButtonText: 'Close', // use '' to get rid of the button
...
//				showCurrentAtPos: 1 // bug; use maxDate instead
	...
	function buildCalendar(classnameContext, options) {
		...
		function onSelectDay(dateText, instance) {
			...
			if (!range.start || range.end) { // start not set, or both already set
				...
				var body = document.querySelector('.ui-priority-primary');
				body.click();
			}
		...
		function beforeShowDay(date) {
			var result = [
					true, // selectable
					range.start && ((+date === +range.start) || (range.end && range.start <= date && date <= range.end)) ? 'ui-state-highlight ' + (range.start && (+date === +range.start) ? 'cstm_class' : '')  + (range.end && (+date === +range.end) ? 'cstm_class123' : '')  : '' // class to be added
				],
				// i
				userResult = [true, '', ''];
	...
	function buildDateRangePicker($originalElement, instance, options) {
		...
		function render() {
			...
			$('.datafrom_custom').append($mask).append($container);
		}
